...
def handle(text, mic):
    ...
    try:
        if any(word in text for word in [u"关机", u"关电脑", u"关闭电脑"]):
            mic.say('即将关闭电脑，请在滴一声后进行确认', cache=True, plugin=__name__, onCompleted=lambda: shutdown(mic.activeListen(MUSIC=True), mic))                        
            ...
            mic.say('即将发送快捷键%s，请在滴一声后进行确认' % button, cache=True, plugin=__name__, onCompleted=lambda: sendHotKey(mic.activeListen(MUSIC=True), mic))
            ...
            mic.say('即将发送指令%s，请在滴一声后进行确认' % cmd, cache=True, plugin=__name__, onCompleted=lambda: command(mic.activeListen(MUSIC=True), mic))            
