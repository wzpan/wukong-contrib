...
class Plugin(AbstractPlugin):
    ...
    def handle(self, text):
        ...
        if self.SLUG not in profile or \
            ...
            return
        pc_email = profile[self.SLUG]['pc_email']
        try:
            if any(word in text for word in [u"关机", u"关电脑", u"关闭电脑"]):
                ...
                if self.SLUG not in profile or \
                    ...
                    return
                button = profile[self.SLUG]['button']
                self.say('即将发送快捷键%s，请在滴一声后进行确认' % button, cache=True, onCompleted=lambda: self.sendHotKey(self.activeListen()))
            elif any(word in text for word in [u"命令", u"指令"]):
                if self.SLUG not in profile or \
                    'cmd' not in profile[self.SLUG]:
                    self.say('您还未设置指令', cache=True)
                    return
                cmd = profile[self.SLUG]['cmd']
                self.say('即将发送指令%s，请在滴一声后进行确认' % cmd, cache=True, onCompleted=lambda: self.command(self.activeListen()))            
