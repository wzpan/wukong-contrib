...
### 测试版（beta）
试用最新版本的 hass 插件，新版 hass 插件支持百度 unit ，实现一句命令完成操作，无需像之前一样先进入插件后操作，由于本插件处于 beta 状态，unit 词典库正在拓充中，暂时使用 apikey 的方式请求 unit的使用
为确保不想使用beta版本的用户的正常使用，本版本与正式版暂时独立
启用本版本需使用如下配置方式：
